...
RSpec.describe WikisController, type: :controller do
    ...
    describe "POST create" do
        before do
            @user = User.create!(email: RandomData.random_email, password: "password")
            allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@user)
        end
        it "increases the number of Wiki by 1" do
            expect{ post :create, params: { user: user, wiki: { title: RandomData.random_sentence, body: RandomData.random_paragraph } } }.to change(Wiki,:count).by(1)
        end
    ...
    describe "PUT update" do
        it "updates wiki with the expected attributes" do
